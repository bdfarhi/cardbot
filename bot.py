    ...
    try:
        with sync_playwright() as p:
            ...
            for attempt in range(max_attempts):
                # Check upfront if button is gone — exit immediately rather
                # than scrolling and waiting for growth that can't happen.
                still_has_button = tab.get_by_role("button", name="Load more").count() > 0
                if not still_has_button:
                    print(f"    'Load more' button gone — fully loaded at {prev_count} cards")
                    break
